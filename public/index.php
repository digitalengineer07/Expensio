...
// Auto-process cron on dashboard load
...
$upcomingRecurring = $recurringModel->getUpcomingTransactionsByUser($user_id, 3);
...
<body class="bg-expensio h-[100dvh] w-full flex font-sans overflow-hidden m-0 p-0">
    ...
    <main class="flex-1 h-[100dvh] overflow-y-auto custom-scrollbar relative pb-24 md:pb-0">
        ...
        <div class="px-6 mb-6 animate-fade">
            <div class="bg-red-50 border border-red-100 rounded-2xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div class="flex items-start sm:items-center gap-3">
                    <div class="w-10 h-10 rounded-full bg-red-100 flex items-center justify-center text-red-500 shrink-0">
                        <i class='bx bx-error-circle text-xl'></i>
                    </div>
                    <div>
                        <h4 class="text-sm font-bold text-red-900">You haven’t added any income yet</h4>
                        <p class="text-xs text-red-700 mt-0.5">You are overspending. Would you like to add an income source?</p>
                    </div>
                </div>
                <a href="wallet.php" class="shrink-0 bg-white text-red-600 border border-red-200 px-4 py-2 rounded-xl text-xs font-bold hover:bg-red-50 transition-colors shadow-sm">
                    + Add Income
                </a>
            </div>
        </div>
        ...
        <section class="px-6 grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade" style="animation-delay: 0.1s;">
            <!-- Total Balance -->
            <?php 
            $is_negative = $total_balance < 0;
            $card_bg = $is_negative ? 'bg-[rgba(255,0,0,0.1)] border border-red-100' : 'glass-panel';
            $text_color = $is_negative ? 'text-[#ff4d4d]' : 'text-gray-900';
            $sign = $is_negative ? '-' : '';
            ?>
            <div class="<?php echo $card_bg; ?> p-6 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] transition-all flex flex-col justify-between rounded-3xl">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="font-bold <?php echo $text_color; ?> text-[15px]">Total balance</h3>
                    <div class="flex items-center cursor-pointer gap-1 border border-gray-200 rounded-full px-2 py-0.5 text-[11px] font-bold text-gray-500 hover:bg-gray-50 transition-colors bg-white">
                        INR <i class='bx bx-chevron-down text-sm'></i>
                    </div>
                </div>
                <h2 class="text-[32px] font-black <?php echo $text_color; ?> mb-6"><?php echo $sign; ?>₹<?php echo number_format(abs($total_balance), 0); ?><span class="<?php echo $is_negative ? 'text-red-400' : 'text-gray-300'; ?> font-medium tracking-tight">.00</span></h2>
                <div class="flex justify-between items-end">
                    <div class="space-y-4">
                        <?php if ($is_negative): ?>
                        <div class="inline-flex items-center gap-0.5 bg-red-100 text-[#ff4d4d] px-2.5 py-1 rounded-lg text-xs font-bold">
                            <i class='bx bx-down-arrow-alt'></i> Overspending
                        </div>
                        <?php else: ?>
                        ...
                        <?php endif; ?>
                        <p class="text-[11px] <?php echo $is_negative ? 'text-[#ff4d4d]' : 'text-gray-400'; ?>">You have extra <span class="font-bold <?php echo $is_negative ? 'text-red-700' : 'text-gray-700'; ?>">+₹1,700</span><br>compared to last month</p>
                    </div>
        ...
        <section class="p-6 grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10 animate-fade" style="animation-delay: 0.2s;">
            ...
            <div class="glass-panel p-6 flex flex-col justify-between overflow-hidden relative shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)]">
                <div class="relative z-10 w-full">
                    ...
                    <?php if ($total_income == 0 && $total_expense > 0): ?>
                        <p class="text-[11px] text-[#ff4d4d] leading-relaxed mb-6 font-semibold">You have spent ₹<?php echo number_format($total_expense, 0); ?> this month without adding income. Consider adding income to balance your budget.</p>
                    <?php else: ?>
                ...
                <div class="mt-6 border-t border-gray-100 pt-4 relative z-10">
                    <h4 class="text-[11px] font-black uppercase text-gray-400 mb-3 flex items-center justify-between">Upcoming Recurring <span class="bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full"><?php echo count($upcomingRecurring); ?></span></h4>
                    <div class="space-y-3">
                        <?php if (empty($upcomingRecurring)): ?>
                            <p class="text-[11px] text-gray-400">No upcoming recurring entries.</p>
                        <?php else: ?>
                            <?php foreach($upcomingRecurring as $r): ?>
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-2">
                                        <div class="w-6 h-6 rounded-full bg-gray-100 flex items-center justify-center text-gray-600 text-xs"><i class='bx bx-refresh'></i></div>
                                        <div>
                                            <p class="text-[11px] font-bold text-gray-900"><?php echo htmlspecialchars($r['description']); ?></p>
                                            <p class="text-[9px] text-gray-500 font-medium"><?php echo date('M d, Y', strtotime($r['next_date'])); ?> (<?php echo ucfirst($r['frequency']); ?>)</p>
                                        </div>
                                    </div>
                                    <span class="text-[11px] font-bold <?php echo $r['type'] === 'income' ? 'text-green-500' : 'text-red-500'; ?>">
                                        <?php echo $r['type'] === 'income' ? '+' : '-'; ?>₹<?php echo number_format($r['amount']); ?>
                                    </span>
                                </div>
                            <?php endforeach; ?>
                        <?php endif; ?>
            ...
            <form action="../app/Controllers/ExpenseController.php?action=create" method="POST" class="space-y-4">
                ...
                <div class="space-y-1.5 mt-2">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="is_recurring" value="1" class="w-4 h-4 text-expensio-purple bg-gray-100 border-gray-300 rounded focus:ring-expensio-purple" onchange="document.getElementById('recurringOptions').classList.toggle('hidden', !this.checked)">
                        <span class="text-[11px] font-black uppercase text-gray-600">Repeat this transaction?</span>
                    </label>
                </div>
                <div id="recurringOptions" class="space-y-1.5 hidden mt-2">
                    <label class="text-[10px] font-black uppercase text-gray-400 ml-1">Frequency</label>
                    <select name="frequency" class="w-full px-4 py-3 bg-white/50 border border-white rounded-2xl outline-none focus:bg-white transition-all text-sm font-medium">
